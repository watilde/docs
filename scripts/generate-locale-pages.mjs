...
function generateJaPage(originalPath, jaPath) {
  ...
  if (extension === '.mdx') {
    ...
    // For TSX/JSX files, only re-export default
    // Note: Next.js Pages Router does not allow `export *` in pages
...
console.log(`\n✨ Generated ${count} Japanese locale pages`);
console.log('📁 Location: src/pages/ja/');
console.log('\n✅ Done! You can now run `yarn dev` or `yarn build`');
